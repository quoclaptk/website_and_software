...
use Modules\Forms\PriceRangeForm;
...
class PriceRangeController extends BaseController
{
    public function onConstruct()
    {
        ...
        $this->view->module_name = 'Khoảng giá tìm kiếm';
    }

    public function indexAction()
    {
        $list = PriceRange::find(
            array(
        ...
        if ($this->request->isPost()) {
            foreach ($paginator->getPaginate()->items as $priceRange) {
                //save sort
                $sortValue = $this->request->getPost('sort_' . $priceRange->id);
                if (!empty($sortValue)) {
                    $priceRange->sort = $sortValue;
                } else {
                    $priceRange->sort = 1;
                }
    ...
    public function createAction()
    {
        ...
        if ($this->request->isPost() && $form->isValid($this->request->getPost())==true) {
            ...
            $data = array(
                ...
                'from_price' => $this->request->getPost('from_price'),
                'to_price' => $this->request->getPost('to_price'),
    ...
    public function updateAction($id, $page = 1)
    {
        ...
        $form = new PriceRangeForm($item, array('edit' => true));
        if ($this->request->isPost() && $form->isValid($this->request->getPost())==true) {
            $save_new = $this->request->getPost('save_new');
            $save_close = $this->request->getPost('save_close');
    ...
    public function deletemultyAction($page = 1)
    {
        ...
        $d = 0;
        foreach ($listid as $id) {
            $item = PriceRange::findFirstById($id);
            if ($item) {
                $item->assign(array(
                    'deleted' => 'N',
                ));
