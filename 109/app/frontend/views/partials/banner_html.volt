{% set item = this.banner_service.getBannerHtml() %}
{% if item != '' %}
{{ stylesheet_link('bannerhtml/' ~ item.subdomain.folder ~ '/' ~ item.id ~ '/style.css?time=' ~ time) }}
{% if setting.banner_1 != '' %}
	{% set bgr = '/files/default/' ~ item.subdomain.folder ~ '/' ~ setting.banner_1  %}
{% else %}
	{% set bgr = '' %}
{% endif %}
...
<div class="container-banner">	
	<div class="bannerbackground" {% if bgr != '' %}style="background:url('{{ bgr }}')"{% else %}style="background:#1e84cc"{% endif %}> 	
		<div class="bannerlogo" style="background: url('/files/default/{{ item.subdomain.folder ~ '/' ~ setting.logo }}') no-repeat center;background-size: 100%;"></div>
			<div class="bannercompany">{{ setting.name }}</div>
			<div class="bannerslogan">{{ setting.slogan }}</div>
			<div class="bannerproduct">
				{% if setting.banner_2 != '' %}
				{{ image('files/default/' ~ item.subdomain.folder ~ '/' ~ setting.banner_2 , 'class':'producth1') }}
				{% else %}
				{{ image('backend/dist/img/no-image.png', 'class':'producth1') }}
				{% endif %}
				{% if setting.banner_3 != '' %}
				{{ image('files/default/' ~ item.subdomain.folder ~ '/' ~ setting.banner_3 , 'class':'producth2') }}
				{% else %}
				{{ image('backend/dist/img/no-image.png', 'class':'producth2') }}
				{% endif %}
				{% if setting.banner_4 != '' %}
				{{ image('files/default/' ~ item.subdomain.folder ~ '/' ~ setting.banner_4 , 'class':'producth3') }}
				{% else %}
				{{ image('backend/dist/img/no-image.png', 'class':'producth3') }}
				{% endif %}
			</div>		
			<div class="bannercontact"> 			
			<div class="bannerhotline">{% if hotline != '' %}Hotline: {{ hotline }} - {% endif %}{% if setting.email != '' %}Mail: {{ setting.email }}{% endif %}</div>
			{% if setting.address != '' %}
