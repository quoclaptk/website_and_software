{% if page.total_pages > 1 %}
    ...
    <ul class="pagination pagination-sm m-t-none m-b-none">
        {% if page.current > 1 %}
            ...
            <li class="prev">{{ link_to(url ~ "/" ~ page_before, 'data-page' : page.before, '<i class="fa fa-angle-left"></i>', 'title' : 'Go to page ' ~ page.last) }}</li>
        {% endif %}
        {% for pageIndex in startIndex..page.total_pages %}
            ...
            <li {% if pageIndex is page.current %}class="active"{% endif %}>
                {{ link_to(url ~ '/' ~ page_index, pageIndex, 'data-page' : pageIndex, 'title' : 'Go to page ' ~ pageIndex) }}
            </li>
        {% endfor %}

        {% if page.current < page.total_pages %}
            <li class="next">{{ link_to(url ~ '/' ~ page.next, 'data-page' : page.next, '<i class="fa fa-angle-right"></i>', 'title' : 'Go to page ' ~ page.next)}}</li>
            <li>{{ link_to(url ~ '/' ~ page.last, 'data-page' : page.last, '<i class="fa fa-angle-double-right"></i>', 'title' : 'Go to page ' ~ page.last) }}</li>
        {% endif %}
    </ul>
{% endif %}