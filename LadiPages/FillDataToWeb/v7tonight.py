...
if __name__ == '__main__':
    ...
    driver.get("http://v7toning.authcomestics.com/?fbclid=IwAR1ffESDUu8W7-D-YGL58o0241_flVLLg3VnWkmfDn7E-Y-cp60De7eOsdk")
    #driver.get("http://myphamnhapkhau.top/sonce05/")
    #driver.save_screenshot('2.png')

    # 4. Scroll to the run-message element to bring it into view (Optional)
    actions = ActionChains(driver)
    actions.move_to_element(driver.find_element_by_css_selector("#HEADLINE1112"))
    actions.perform()
    time.sleep(3)
    ...
    while True:
        ...
        Select(driver.find_element_by_name('select1')).select_by_index(random.randint(1, 5))
        # quà
        qua = driver.find_element_by_name('lpinput1')
        qua.clear()
        qua.send_keys("Chúc bạn buôn may bán đắt, gặt hái được nhiều thành công. Vui lòng liên hệ 0963836272 gặp quân để tìm được nguồn hàng giá tốt")

        street = driver.find_element_by_name('street')
        street.clear()
        street.send_keys("Liên hệ để được qua số điện thoại để gặp mình")
        
        driver.find_element_by_id('BUTTON2025').click()
        time.sleep(3)
